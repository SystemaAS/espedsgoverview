...
public class DashboardController {
	...
	public ModelAndView logon(@ModelAttribute (AppConstants.SYSTEMA_WEB_USER_KEY) SystemaWebUser appUser, BindingResult bindingResult, HttpSession session, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttr){
		...
		String uopp = request.getParameter("uopp");
		if(strMgr.isNull(uopp)){
			successView = new ModelAndView("sendingerlevtidgatefilter");
		}
		...
		if(appUser==null){
		    ...
		    if(bindingResult.hasErrors()){
		    	...
		    	//All sub-modules will be passed an encrypted userid/password (from the dashboard). ALWAYS!
	...
	private String getTomcatServerRedirectionUrl(SystemaWebUser appUser, HttpServletRequest request){
		...
		try{
			if(strMgr.isNotNull(uopp)){
				//Ufortoppdr.
				retval = hostRaw + request.getContextPath() + "/logonWRedDashboard.do?uopp=1" + "&ru=" + URLEncoder.encode(appUser.getEncryptedUser(), "UTF-8") + "&dp=" + URLEncoder.encode(appUser.getEncryptedPassword(), "UTF-8");
			}else{
				//Sendinger..-
